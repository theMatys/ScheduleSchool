...
/// Holds all the animation types for views
struct SSViewAnimation
{
    /// Executes a set of actions while animating with the given duration, delay and timing function.
    ///
    /// This animation acts like a simultanious animation group, therefore, when given a delay, all animations will be executed with the same delay all at once.
    class Basic: SSAnimation
    {
        // MARK: Implemented properties from: SSAnimation
        var duration: TimeInterval
        var delay: TimeInterval?
        var timingCurve: SSCubicTimingCurve?
        ...
        /// The set of actions which are executed after the animation has been completed.
        var completion: ((UIViewAnimatingPosition) -> ())?
        
        /// The "`UIViewPropertyAnimator`" representation of the animation.
        internal var representedAnimation: UIViewPropertyAnimator!
        
        /// The set of actions which should be animated.
        private var animations: [() -> ()]
        
        // MARK: - Initializers
        
        /// Initializes an `SSViewAnimation.Basic` object with no animations and no duration.
        init()
        {
            duration = 0.0
            animations = [() -> ()]()
        ...
        func commit()
        {
            prepareRepresentedAnimation()
            representedAnimation.startAnimation(afterDelay: delay ?? 0.0)
        }
        
        // MARK: Methods
        
        /// Prepares the represented animation before it is executed.
        internal func prepareRepresentedAnimation()
        {
            representedAnimation = UIViewPropertyAnimator(duration: duration, timingParameters: timingCurve?.uiCubicTimingParameters ?? SSCubicTimingCurve.linear.uiCubicTimingParameters)
            
            for animation in animations
            {
                representedAnimation.addAnimations(animation)
            }
            
            if(completion != nil)
            {
                representedAnimation.addCompletion(completion!)
            }
        }
    }
}

// MARK: - Stackable setter extension (SSViewAnimation.Basic)
extension SSViewAnimation.Basic
{
    /// Adds a set of actions to the animation which should also be animated.
    ///
    /// - Parameter additionalAnimations: The set of actions which should be added to the animation.
    /// - Returns: The class (to enable stackable functions).
    func with(additionalAnimations: @escaping () -> ()) -> Self
    {
        animations.append(additionalAnimations)
        return self
    }
    
    /// Defines a set of actions which are executed after the animation has been completed.
    ///
    /// - Returns: The class (to enable stackable functions).
    func withCompletion(_ completion: @escaping (UIViewAnimatingPosition) -> ()) -> Self
    {
